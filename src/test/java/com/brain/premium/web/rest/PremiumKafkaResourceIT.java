...
import com.brain.premium.config.KafkaProperties;
...
class PremiumKafkaResourceIT {
    ...
    void setup() {
        ...
        PremiumKafkaResource kafkaResource = new PremiumKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/premium-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/premium-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
